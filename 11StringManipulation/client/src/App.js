...
function App() {
  ...
  const[lengthInput, setLengthInput] = useState(undefined);
  ...
  async function calculateLength(e) {debugger;
    e.preventDefault();
    alert(lengthInput);
  ...
  async function concatenate(e) {
    e.preventDefault();
    const concat = await contract.methods.concatenate(
      e.target.elements[0].value,
      e.target.elements[1].value
    ).call();
    setConcatenation(concat);
  }
        ...
        <div className="col-sm-12">
          ...
          <form onSubmit={e =>calculateLength(e)}>
            <div className="form-group">
              ...
              <input name="length" onChange={e => setLengthInput(e.target.value)} type="text" className="form-control" id="string-length" />
            </div>
